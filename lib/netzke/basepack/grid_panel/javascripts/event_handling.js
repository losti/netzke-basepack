{
  ...
  onApply: function(){
    if (this.fireEvent('apply')) {
      ...
      if (!this.saveMask) this.saveMask = new Ext.LoadMask(this.getEl(), {msg: this.saveMaskMsg});
      this.saveMask.show();
  ...
  onItemContextMenu: function(grid, record, item, rowIndex, e){
    ...
    if (!grid.getSelectionModel().isSelected(rowIndex)) {
      if (!grid.getSelectionModel().selectRow) return;
