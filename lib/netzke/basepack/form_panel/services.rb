      ...
      module Services
        ...
        private
          ...
          def create_or_update_record(hsh)
            ...
            @record ||= data_adapter.find_record hsh.fetch(data_class.primary_key.to_s,nil) # only pick up the record specified in the params if it was not provided in the configuration
              #data_class.find(:first, :conditions => {data_class.primary_key => hsh.delete(data_class.primary_key)}) 
            success = true

            hsh.delete(data_class.primary_key.to_s) if @record
